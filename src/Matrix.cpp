...
{//operacion multiplicacion por punto
    Matrix A(3, 1);
    A.aij[0][0] = P.x;
    A.aij[1][0] = P.y;
    A.aij[2][0] = P.z;
...
void Matrix::definir()
{
    std::cout << "Creando Matriz " << std::endl;
    ...
    if (n > 0 && m > 0) {
        ...
        for (int i = 0; i < n; i++)
            for (int j = 0; j < m; j++) {
                std::cout << "introduce ij (" << i + 1 << "," << j + 1 << ")" << std::endl;
                std::cin >> ij(i, j);
    ...
    if (this->n > 0 && this->m > 0) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                std::cout << ij(i, j) << "  " << std::flush;
            }
            std::cout << "\n" << std::endl;
        }
    }
}
double & Matrix::ij(int i, int j)
{ //devuelve la direccion
    if (i >= 0 && j >= 0 && i < n && j < m)
        return aij[i][j]; //c++ numera las ijs de una matrix a partir de cero, nosotros a partir de 1
...
Matrix Matrix::operator+(const Matrix &A)
{
    if (n > 0 && m > 0) {
        if (n == A.n && m == A.m) {
            ...
            Matrix B(1, 1);
            std::cout << " Tama�os diferentes" << std::endl;
            return B;
        };
