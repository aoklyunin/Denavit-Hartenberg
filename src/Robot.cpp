...
void Robot::inicializar()
{
    ...
    b2->leer("models/b2.STL");
    b3->leer("models/b3.STL");
    b4->leer("models/b4.STL");
    b5->leer("models/b5.STL");
    b6->leer("models/b6.STL");
    gripe->leer("models/gripe.STL");
...
void Robot::Drawarrow3D(vector3d A, vector3d B, vector3d color, double cota1, double R)
{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        glVertex3f(vertex[i].x + B.x - fraccion * (B.x - A.x),
                   vertex[i].y + B.y - fraccion * (B.y - A.y),
                   vertex[i].z + B.z - fraccion * (B.z - A.z));
